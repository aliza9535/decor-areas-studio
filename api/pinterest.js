...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');const action=String(req.query.action||'');const c=cookies(req);
  ...
  if(action==='sandbox-setup'){
    ...
    const desiredDescription='Sandbox board for user-approved API testing.';

    // Pinterest Sandbox can return empty board pages even when boards exist, so walk bookmarks.
    ...
    // Remove old PinScope branding from any Sandbox board created by the previous build.
    ...
    // Never expose legacy PinScope board names in the app UI. Put the review board first.
