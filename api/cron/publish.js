...
export default async function handler(req,res){
  ...
  const due=await sql`select * from scheduled_pins where status='scheduled' and approved_at is not null and scheduled_at<=now() order by scheduled_at asc limit 25`,results=[];
  for(const pin of due){
    const claim=await sql`update scheduled_pins set status='publishing' where id=${pin.id} and status='scheduled' returning id`;if(!claim.length)continue;
    try{
