...
export async function upsertPinterestAccount(userId,data){
  ...
  await sql`insert into pinterest_accounts(user_id,username,access_token,refresh_token,access_expires_at,refresh_expires_at,scopes,updated_at)
    ...
    on conflict(user_id) do update set
      ...
      refresh_expires_at=coalesce(excluded.refresh_expires_at,pinterest_accounts.refresh_expires_at),
...
export async function getPinterestAccount(userId){
  ...
  await ensureSchema();
  const sql=getDb();
  const rows=await sql`select * from pinterest_accounts where user_id=${userId} limit 1`;
  return rows[0]||null;
}
export async function getFreshAccessToken(userId){
  const acct=await getPinterestAccount(userId);
  if(!acct) throw new Error('Pinterest is not connected for this account.');
  ...
  const sql=getDb();
  const accessExpiry=new Date(Date.now()+Number(d.expires_in||2592000)*1000);
