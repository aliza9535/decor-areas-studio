...
export default async function handler(req,res){
  ...
  const access=env==='sandbox'?'da_sandbox_access':'da_access',refresh=env==='sandbox'?'da_sandbox_refresh':'da_refresh',out=[authCookie(access,seal(d.access_token,secret),d.expires_in||2592000),clearCookie('da_oauth_state'),clearCookie('da_oauth_env'),clearCookie('da_oauth_redirect')];if(d.refresh_token)out.push(authCookie(refresh,seal(d.refresh_token,secret),d.refresh_token_expires_in||5184000));res.setHeader('Set-Cookie',out);
  if(env==='production'){
    try{
      const user=await getSessionUser(req);
      if(user){
        let username=null;const ar=await fetch('https://api.pinterest.com/v5/user_account',{headers:{Authorization:'Bearer '+d.access_token,Accept:'application/json'}});if(ar.ok){const ad=await ar.json();username=ad?.username||null}
