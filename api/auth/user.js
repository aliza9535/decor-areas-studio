...
export default async function handler(req,res){
  ...
  if(action==='signup'){
    ...
    const id=crypto.randomUUID();await sql`insert into app_users(id,email,password_hash) values(${id},${email},${hashPassword(password)})`;
    const token=await createSession(id);res.setHeader('Set-Cookie',sessionCookie(token));
  ...
  if(action==='login'){
    ...
    const token=await createSession(user.id);res.setHeader('Set-Cookie',sessionCookie(token));
