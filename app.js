...
const state={fileData:null,fileType:null,remoteImage:null,user:null,dbConfigured:false,productionConnected:false,sandboxConnected:false,boards:[],schedule:[],article:null,billing:null,analytics:null,pins:[]};
const pageMeta={
  dashboard:['Dashboard','Content operations at a glance'],analytics:['Analytics','Authorized organic performance'],create:['Create Pin','Publish now or schedule'],scheduler:['Scheduler','Approved future publishing'],blog:['Blog → Pin','Turn your article into a reviewed Pin'],integrations:['Integrations','Connections and safeguards'],plans:['Plans','Subscriptions and queue capacity'],settings:['Settings','Account, authorization and data controls']
};
...
function show(name){
  ...
  if(name==='analytics'){loadAnalytics(true);loadPins()} if(name==='scheduler')loadSchedule(); if(name==='integrations')loadIntegrations(); if(name==='plans')loadBilling(); if(name==='settings')refreshSettings();
...
let authMode='login';
$$('.auth-tab').forEach(b=>b.onclick=()=>{authMode=b.dataset.auth;$$('.auth-tab').forEach(x=>x.classList.toggle('active',x===b));$('#authSubmit').textContent=authMode==='signup'?'Create account':'Sign in';$('#authPassword').autocomplete=authMode==='signup'?'new-password':'current-password'});
...
async function loadStatus(){
  try{
    const s=await api('/api/pinterest?action=status');state.productionConnected=!!s.productionConnected;state.sandboxConnected=!!s.sandboxConnected;
    ...
    if(state.productionConnected) await loadBoards(); else{$('#board').innerHTML='<option value="">Connect Pinterest first…</option>';$('#publish').disabled=true;$('#schedulePin').disabled=true}
...
async function loadBoards(){
  try{
    const d=await api('/api/pinterest?action=boards');state.boards=d.boards||[];$('#board').innerHTML=state.boards.map(b=>'<option value="'+esc(b.id)+'">'+esc(b.name||b.id)+'</option>').join('')||'<option value="">No Boards returned</option>';
    $('#publish').disabled=!state.boards.length;$('#schedulePin').disabled=!state.boards.length;
  }catch(e){$('#board').innerHTML='<option value="">Could not load Boards</option>'}
}
...
$('#imageFile').onchange=()=>{
  const f=$('#imageFile').files[0];if(!f)return;if(!['image/jpeg','image/png','image/webp'].includes(f.type)){toast('Use JPG, PNG or WebP');return}if(f.size>3*1024*1024){toast('Use an image under 3 MB');return}
...
function renderSchedule(items){
  ...
  $('#scheduleList').innerHTML=sorted.map(x=>{const d=new Date(x.scheduled_at);return '<div class="timeline-item"><div class="timeline-time">'+d.toLocaleDateString([], {month:'short',day:'numeric'})+'<br>'+d.toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})+'</div><div class="timeline-line"></div><div class="timeline-card"><div class="thumb" '+(x.image_url?'style="background-image:url(&quot;'+esc(x.image_url)+'&quot;)"':'')+'></div><div><strong>'+esc(x.title)+'</strong><small>'+esc(x.board_name||'Board')+(x.last_error?' · '+esc(x.last_error):'')+'</small></div><span class="status-mini '+esc(x.status)+'">'+esc(x.status)+'</span></div><div class="timeline-actions">'+(['scheduled','failed'].includes(x.status)?'<button class="secondary cancel-pin" data-id="'+esc(x.id)+'">Cancel</button>':'')+'</div></div>'}).join('')||'<div class="empty-state">No scheduled Pins yet.</div>';
  $$('.cancel-pin').forEach(b=>b.onclick=async()=>{if(!confirm('Cancel this scheduled Pin?'))return;try{await api('/api/schedule?id='+encodeURIComponent(b.dataset.id),{method:'DELETE'});await loadSchedule();toast('Scheduled Pin cancelled')}catch(e){toast(e.message)}})
}
$('#refreshSchedule').onclick=loadSchedule;

$('#importArticle').onclick=async()=>{
  const n=$('#importNotice'),url=$('#articleUrl').value.trim();if(!url){n.className='notice error';n.textContent='Paste a public article URL.';return}
  n.className='notice';n.textContent='Importing public article metadata…';
  try{const d=await api('/api/blog?url='+encodeURIComponent(url));state.article=d.article;n.className='notice success';n.textContent='Article imported. Review the result before using it.';
...
async function loadBilling(){
  try{
    ...
    const plan=d.user?.plan||state.user?.plan||'free';$$('.checkout').forEach(b=>{b.disabled=!d.stripeConfigured||!state.user;b.textContent=b.dataset.plan===plan?'Current plan':'Choose '+b.dataset.plan[0].toUpperCase()+b.dataset.plan.slice(1)});
...
async function init(){
  setDefaultSchedule();await loadUser();await loadStatus();await loadAccount();await Promise.all([loadAnalytics(false),loadSchedule(),loadBilling()]);await loadPins();await syncCheckout();
...
window.addEventListener('resize',()=>{if(state.analytics){drawLine($('#trendChart'),state.analytics.series,$('#trendEmpty'));drawLine($('#analyticsChart'),state.analytics.series,$('#analyticsTrendEmpty'))}});
init();
